...
RSpec.describe Api::V1::MembershipsController, type: :request do
  ...
  describe 'POST #create' do
    context 'when board and new user exist' do
      ...
      subject { post api_v1_board_memberships_path(board_id: board1.id, user_id: user1.id) }
    ...
    context 'when params invalid' do
      subject { post api_v1_board_memberships_path(board_id: board.id, user_id: user.id) }
  ...
  describe 'PATCH #admin' do
    context 'when the board has admin and new user exist' do
      subject { patch admin_api_v1_board_membership_path(board_id: board.id, id: user.id), params: { user_id: user.id } }
    ...
    context 'when the user is membership and has admin rights' do
      ...
      subject { patch admin_api_v1_board_membership_path(board_id: board.id, id: user1.id), params: { user_id: user1.id } }
    ...
    context 'when board not have admin member' do
      subject { patch admin_api_v1_board_membership_path(board_id: board1.id, id: user1.id), params: { user_id: user1.id } }
